...
function accessId(){ // 함수 선언
    ...
    // 2) div1의 배경색을 변수에 저장
    // JS에서 CSS과년 속성을 다룰때는 style을 이용함
...
function accessName(){
    ...
    // 3) for문 이용하여 hobbyList 요소 반복 접근
...
// cssTest()로 접근
function cssTest(){
    // querySelector("선택자") : 요소 1개 선택 (여러 요소가 존재하면 첫 번째 요소 선택)
    // 1개만 선택되는 요소
...
function readValue(){
    ...
    //input의 값이 비어있지 않다면 문자열.trim() : 문자열 양쪽 공백 제거
