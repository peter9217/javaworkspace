...
<body>
            ...
            <!-- 이미지가 있을 경우 -->
                <c:if test="${not empty board.imageList}" >
                    ...
                    <%-- 
                        ...
                        - IMG_ORDER의 값이 0인 이미지가 컴네일이다 
                        => imageList에 썸네일이 있다면 조회 되었을 때 IMG_ORDER가 0인 이미지가 imageList[0]에 저장 되었을 것이다.
                    --%>
    ...
    <%-- 20230510 boardDetail.jsp 맨 밑 --%>
    
    <%-- 누가(로그인한 회원의 번호) 어떤 게시글(현재 게시글 번호) 좋아요를 클릭/취소 --%>
    <%-- 
        로그인한 회원 번호 얻어오기
        1) ajax로 session에 있는 loginMember의 memberNo를 반환
        2)  HTML 요소에 로그인한 회원의 번호를 숨겨 놓고 JS로 얻어오기
        3) jsp 파일 제일 위에 있는 script 태그에 JS + EL 이용해서 전역 변수로 선언해두기
    --%>
    <script>
        // JSP에서 작성 가능한 언어/ 라이브러리 : html, css, js, java, EL, JSTL
        //  JSP 해석 우선 순위 : Java/ JSTL -> HTML, CSS, JS
