...
//	@PostMapping("/login")
	public String login(/*@RequestParam("inputEmail")*/ String inputEmail, /*@RequestParam("inputPw")*/ String inputPw) {
		...
		// 파라미터 전달 방법 2: @RequestParam 어노테이션 이용(+생량 방법)
		...
		// @RequestParam 어노테이션
		   ...
		   // - 매개변수 앞에 해당 어노테이션을 작성하면, 매개변수에 값이 주입됨.
			...
			//@RequestParam(value="name", required="false", defailtValue="1")
		   // [속성]
...
//	@PostMapping("/login")
	public String login(/*@ModelAttribute*/ Member inputMember) {
		// 파라미터 전달 방법 3: @ModelAttribute 이용 방법
	...
	public String login(Member inputMember, Model model
		...
		// @RequestHeader(value="referer") : 요청 HTTP header에서 "referer"(이전 주소) 값을 얻어와 매개 변수 String referer에 저장
		// Model model : 데이터 전달용 객체 -> 데이터를 K : V 형식으로 담아 전달
		// 기본적으로 request scope -> @SessionAttributes 어노테이션과 함께 사용 시 Session scope
		// @RequestParam(value="saveId", required=false) String saveId
		// 		-> name 속성 값이 "saveId"인 파라미터를 전달 받아 저장 -> required=false : 필수 아님(null 허용)
		// 		(주의) required 속성 미작성 시 기본 값 ture -> 파라미터가 전달되지 않는 경우 주의 
...
//		System.out.println(loginMember);
		...
		if(loginMember != null) { // 로그인 성공 시
			...
			// 2) 클래스 위에 @SessionAttributes 추가 -> session scope로 변경
			...
			/* redirect(재요청) 시
			 * 기존 요청(request)이 사라지고 새로운 요청을 만들게 되어 redirect된 페이지에서는 이전 요청이 유지 되지 않는다.
			 * -> 유지하고 싶으면 어쩔 수 없이 Session을 이용 
			 * 
			 * [Sptring]
			 * 이런 상황을 해결하기 위한 객체 RedirectAttibutes를 제공
			 * 
	...
	public String logout(SessionStatus status) {
		...
		// 세션 무효화 servlet -> HttpSession.invalidate()
		// Spring
...
//			path += "/member/signUp"; // 절대 경로
	 ...
	 * 
	 * 1 순위 : 메서드 단위로 처리 -> try-catch / throws
	 * 
	 * 2 순위 : 클래스 단위로 처리 -> @ExceptionHandler
	 * 
	 * 3 순위 : 프로그램 단위(전역) 처리 -> @ControllerAdvice
	 * */
	...
	public String exceptionHandler(Exception e, Model model) {
		...
		// forward 진행 -> Veiw Resolver의 prefix, suffix를 붙여 JSP 경로로 만듦
		return "common/error";
	}
