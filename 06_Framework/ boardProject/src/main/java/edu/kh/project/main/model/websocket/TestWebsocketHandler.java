...
public class TestWebsocketHandler extends TextWebSocketHandler{
	// websocketSession 클라이언 - 서버간 전이중통신을 담당하는 객체
	// 클라이언트의 세션을 가로채서 저장하고 있음
	// Set : 중복 X, 순서 X
	// synchronizedSet : 동기화된 Set 객체 반환 -> 멀티쓰레드(줄 안서고 기능들이 서로 실행되려고 함)
	// 환경에서 스레드간의 의도치 않은 출동을 예방하기 위해서 동기화(줄세움) 
...
//	- 클라이언트와 연결이 완료되고, 통신할 준비가 되면 실행
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		// 클라이언트 웹소켓 연결을 요청하면 sessions에 클라이언트와의 전이중 통신을 담당하는 객체(WebSocketSession)를 추가
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		// /testSock으로 연결된 객체를 만든 클라이언트들에게 전달 받은 내용을 보냄
