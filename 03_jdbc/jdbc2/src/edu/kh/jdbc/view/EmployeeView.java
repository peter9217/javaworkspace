...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				// EMT_YN, ENT_DATE 수정  'Y' 'SYSDAE로 수정
				...
				System.out.println("\n[잘못된 입력입니다.]\n");
				sc.nextLine(); // 입력 버퍼 문자열 제거
				input = -1; // while문 종료되지 않게 하기 위한 값
			}
		...
		try {
			...
			for (Employee emp : empList) {
				System.out.printf("%d / %s / %s / %s / %s\n", emp.getEmpId(), emp.getEmpName(),
						emp.getDepartmentTitle(), emp.getJobName(), emp.getPhone());
			}
	...
	private void selectName() {
		...
		List<Employee> list;
		try {
			list = service.selectName(input);
			if (list.isEmpty()) { // 조회된 사원이 없을 경우
				System.out.println("[일치하는 글자가 포함된 사원이 존재하지 않습니다.]");
				return;
			}
	...
	private void selectSalary() {
		...
		try {
			List<Employee> list = service.selectSalary(first, second);
			if (list.isEmpty()) { // 조회된 사원이 없을 경우
				System.out.println("[급여 범위에 포함된 사원이 존재하지 않습니다.]");
				return;
			}
			for (Employee emp : list) {
	...
	private void retireEmployee() {
		...
		int input = sc.nextInt();
		System.out.println("정말 퇴사 처리하시겠습니까?(Y/N)");
		...
		try {
			...
			String str = null;
			if(result>0) str = "퇴사 처리 완료";
			else		 str = "사번이 일치하는 사원 ㅣ없습ㄴ디ㅏ";
			System.out.println(str);
		} catch (SQLException e) {
			System.out.println("퇴사 처리 예외 발생");
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
