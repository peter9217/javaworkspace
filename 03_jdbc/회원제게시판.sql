...
-- 회원(MEMBER) 테이블
--CREATE TABLE MEMBER(MEMBER_NO NUMBER NOT NULL, 
--MEMBER_NO VARCHAR2(30) CONSTRAINT MEMBER_PK PRIMARY KEY, 
--MEMBER_ID VARCHAR2(30) NOT NULL, 
--MEMBER_PW VARCHAR2(30) NOT NULL, 
--MEMBER_NM VARCHAR2(30) NOT NULL, 
--MEMBER_GENDER CHAR(1) CONSTRAINT GENDER_CHK CHECK(MEMBER_GENDER IN ('M','W')), 
...
INSERT INTO "BOARD" VALUES(SEQ_BOARD_NO.NEXTVAL, '샘플 제목 1', '내용1 입니다.\N안녕하세요', DEFAULT, DEFAULT, DEFAULT, 1);
INSERT INTO "BOARD" VALUES(SEQ_BOARD_NO.NEXTVAL, '샘플 제목 2', '내용2 입니다.\N안녕하세요', DEFAULT, DEFAULT, DEFAULT, 1);
INSERT INTO "BOARD" VALUES(SEQ_BOARD_NO.NEXTVAL, '샘플 제목 3', '내용3 입니다.\N안녕하세요', DEFAULT, DEFAULT, DEFAULT, 2);
...
--SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, READ_COUNT, CREATE_DT FROM "BOARD" JOIN "MEMBER" USING(MEMBER_NO) JOIN "COMMENT" USING(BOARD_NO) /*WHERE BOARD_NO != 0 INDEX*/ ORDER BY BOARD_NO DESC;
--SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT FROM "BOARD" LEFT JOIN "MEMBER" USING(MEMBER_NO) LEFT JOIN "COMMENT" USING(BOARD_NO) GROUP BY BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT ORDER BY BOARD_NO DESC;
--SELECT * FROM "BOARD" LEFT JOIN "MEMBER" USING(MEMBER_NO) LEFT JOIN "COMMENT" USING(BOARD_NO) ORDER BY BOARD_NO DESC;

-- 게시글 목록 조회
-- 게시글번호, 제목, 작성자(이름), 작성일, 조회수
-- 최신 게시글이 위쪽으로 오도록 정렬해서 조회
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT FROM "BOARD" JOIN "MEMBER" USING(MEMBER_NO) /*WHERE BOARD_NO != 0 INDEX*/ ORDER BY BOARD_NO DESC; 
-- 게시글 목록 조회
-- 게시글번호, 제목, 작성자(이름), 작성일, 조회수
-- 최신 게시글이 위쪽으로 오도록 정렬해서 조회
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT FROM "BOARD" JOIN "MEMBER" USING(MEMBER_NO) /*WHERE BOARD_NO != 0 INDEX*/ ORDER BY BOARD_NO DESC; 
...
-- 게시글번호, 제목, 작성자(이름), 작성일, 조회수
-- 최신 게시글이 위쪽으로 오도록 정렬해서 조회
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, CREATE_DT, READ_COUNT, (SELECT COUNT(*) FROM "COMMENT" C WHERE C.BOARD_NO = B.BOARD_NO AND DELETE_FL = 'N') COMMENT_COUNT FROM "BOARD" B JOIN "MEMBER" USING(MEMBER_NO) WHERE DELETE_FL = 'N'/*WHERE BOARD_NO != 0 INDEX*/ ORDER BY BOARD_NO DESC; 
SELECT * FROM "BOARD"
...
----------------------------------------------------------------------------
----------------------------------------------------------------------------
UPDATE "MEMBER" SET MEMBER_NM = '123?', MEMBER_GENDER = 'M' WHERE MEMBER_NO = '123';
...
-- 게시글 상세 조회
SELECT BOARD_NO, BOARD_TITLE, BOARD_CONTENT, MEMBER_NO, MEMBER_NM, READ_COUNT, CREATE_DT FROM "BOARD" JOIN "MEMBER" USING(MEMBER_NO) WHERE DELETE_FL = 'N' AND BOARD_NO = ?;

-- 조회 수 증가(BOARD 테이블 READ_COUNT)
...
-- 게시글 삭제 UPDATE 
UPDATE "BOARD" SET DELETE_FL = 'Y' WHERE BOARD_NO = ?;
...
FROM "COMMENT" JOIN "MEMBER" USING(MEMBER_NO) WHERE DELETE_FL = 'N' AND BOARD_NO = 1 ORDER BY COMMENT_NO;
SELECT * FROM "COMMENT";
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, ?, DEFAULT, DEFAULT, ?, ?); 
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '댓글 샘플 3번', DEFAULT, DEFAULT, 3, 2); 
UPDATE "COMMENT" SET COMMENT_CONTENT = ? WHERE COMMENT_NO = ?; 
UPDATE "COMMENT" SET DELETE_FL = 'Y' WHERE COMMENT_NO = 1; 
SELECT COMMENT_NO FROM "COMMENT" WHERE BOARD_NO  = 2; 