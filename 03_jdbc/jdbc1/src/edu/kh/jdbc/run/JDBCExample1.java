...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		// JDBC : JAVA에서 DB에 연결(접근)할 수 있게 해주는 코드(class, interface)를 제공해 주는 것
		// Java Programming API java.sql 패키지에서 제공

		/*
		 * ** JDBC를 이용한 애플리케이션 제작 시 필요한 것 **
		 * 
		 * 1. Java의 JDBC 관련 인터페이스 2. DBMS의 종류(Oracle) 3. DBMS 회사에서 제공하는 Java 애플리케이션과 DB
		 * 연결을 위한 라이브러리 (ojdbc11.jar -> OracleDriver.class)
		 * 
		 */
		...
		// DB연결 정보를 담는 객체 --> Connection을 상속 받아 구현한 자식 객체가 대입될 예정
		// DBMS 이름, 타입, IP, PORT, 계정명, 비밀번호 저장 -> DBeaver의 계정 접속 방법 추가와 유사함
		// 왜 Connection은 인터페이스일까?? -> DBMS마다 연결 정보, 방법이 다르기 때문에 상속을 받아 
		// 공통된 부분을 DBMS마다 알맞게 구현하고 구현된 객체를 Java에서 참조해서 사용 
		// 
		
		Statement stmt = null;
		// sql을 DB에 전달해 수행하고 결과(ResultSet 또는 DML 성공 행의 개수)를 반환 받는데
		// 사용되는 객체 
		
		
		ResultSet rs = null;
		// SELECT 질의 성공 시 반환되는 객체 조회 결과의 집합을 나타냄 -> 테이블 모양으로 반환됨
		// -> ResultSet 객체를 이용해서 1행씩 접근, 더 이상 접근할 행이 없을 경우 false 반환
		 ...
		 try {
			 ...
			 // ClassNotFoundException 발생 가능성이 있음. (오타 또는 라이브러리 추가 x시)
			 ...
			 // 19c 버전 : :ORCL
			 // 18c 또는 강의장 DB : :XE
			 String user = "kh_jbs"; // 계정명
			 
			 String pw = "oracle_jbs123A";
			 
			 // DriverManager 객체
			 // - 메모리에 로드된 JDBC 드라이버 객체를 이용해 DB에 접근하고 작성된 정보를 이용해
...
//			 3. SQL작성 
			 ...
			 // -------------------3단계 : SQL 수행 결과로 반환 받은 ResultSet을 첫 번째 행부터 1행씩 접근하여 컬럼 값을
			 // 		얻어와 출력
			 
			 while(rs.next()) {
				 // - rs.next() - ResultSet 객체는 조회 결과를 한번에 테이블로 다루는 것이 아닌
