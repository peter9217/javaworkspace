...
function selectCommentList(){
    ...
    //   - 자원을 이름으로 구분(REpresentational)하여 자원의 상태(State)를 주고 받는 것 (Transfer)
    // -> 주소를 명시하고 Http Method(GET, POST, PUT, DELETE)를 이용해 지정된 자원에 대한 CRUD 진행
    // Create : 생성, 삽입(POST)
    // Read  : 조회 (GET)
    // Update  : 수정 (PUT, PETCH)
    // DELETE   : 삭제 (DELETE)
...
function insertChildComment(parentNo, btn){
    ...
    const data = {"commentContent" : commentContent, "memberNo": loginMemberNo, "boardNo" : boardNo, "parentNo" : parentNo};
    fetch("/comment", {
        method : "POST",
        headers : {"Content-Type" : "application/json"}, 
        body : JSON.stringify(data) // JS객체 -> JSON 파싱
