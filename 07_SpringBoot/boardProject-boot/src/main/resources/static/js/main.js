...
btn1.addEventListener("click", () => {
    ...
    fetch("/selectMemberTel?nickname=" + inputNickname.value) // GET 방식 요청 (파라미터를 쿼리스트링으로 추가)

    .then(resp => resp.text())
    //resp : 응답 객체 // resp.text() : 응답 객체 내용을 문자열로 변환하여 반환
...
//     obj.str = str;
//     // console.log(obj)
//     testSock.send(JSON.stringify(obj)) // 웹소켓 연결된 곳으로 메시지를 보내ㅁ
//     // JS객체 -> JSON
...
function getCookie(key){
    ...
    // saveOd
    // 배열.map() : 배열의 모든 순차 접근하여 함수 수행 후 수행 결과를 이용해서 새로운 배열을 만드는 함수
