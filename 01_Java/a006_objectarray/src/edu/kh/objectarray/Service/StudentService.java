...
public class StudentService {
	...
	public boolean addStudent(int grade, int classRoom, int number, String name) {
		...
		// studentArr에서 비어있는 인덱스를 찾아 해당 인덱스에 매개변수를 이용해서 생성된
		// Student 객체 주소를 대입
		// -> true 반환
		for(int i=0; i<studentArr.length; i++) {
			//배열 요소가 참조하는 주소가 없을 경우 == 비어있다고 판단
			if(studentArr[i] == null) {
				//비어이쓴ㄴ 배열 요소에 매개변수를 이용해서 새 학생 객체를 만들어 주소 대입
	...
	public Student selectIndex(int index) {
		...
		// 배열 범위를 초과 했다는 ArrayIndexOfBoundsException 발생
	...
	public Student[] selectName(String inputName) {
		...
		int j = 0; // resultArr에 값을 대입할 인덱스를 나타낸 변수
		...
		for(int i=0; i<studentArr.length; i++) {
			...
			if(studentArr[i].getName().equals(inputName)) {
				...
				resultArr[j++] = studentArr[i];
				// -> studentArr[i] (후위 연산)
			}
			
		}
		// 이름이 일치하는 학생이 ㅓㅄ어서 index가 증가하지 않았다면
		if(j == 0) {
			return null; // null 반환
		}
