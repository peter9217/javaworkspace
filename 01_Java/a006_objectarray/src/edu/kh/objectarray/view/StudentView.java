...
public class StudentView {
	...
	private void selectAll() {
		...
		//service가 참조하는 객체의 필드 중 studentArr를 반환 받아 각 배열 요소를 화면에 출력
		...
		for(int i=0; i<studentList.length; i++) {
			...
			//Student 객체가 가지고 있는 toString()메서드를 호출해서 학생
			// 정보를 문자열로 반환 받아 출력
			...
			//NullpointerException -> 참조 하고 있는 객체가 없는 참조변수를 이용해서
	...
	private void selectIndex() {
		...
		//받환 받은 index번째 요소가 null이 아닐경우 (학생 정보 있을 경우)
	...
	private void selectName() {
		...
		if(resultArr != null) {
			...
			for(Student s : resultArr) {
				...
				if(s==null) break;
				// if/else if/else/for/while 등 {} 블록을 작성하는 구문에서 {}블록을 생략하면 
				// 다음 한 줄에만 해당 구문을 적용시킨다. (한 줄의 기준은 세미콜론 전까지)
