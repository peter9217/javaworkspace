...
//		c1.getDiscountOffer();
	...
	public void ex3() {
		...
		for(Car c : carList) {
			...
			System.out.println("연료 형식 수 : " + c.getFuel());
			//Truck일 경우 최대 적재 하중 : 2.5t
			//LightCar일 경우 할인율 : 30%
			//Car일 경우 -> 차종이 등록되어 있지 않습니다.
	...
	public void ex4() {
		// 객체배열 + 다형성(업캐스팅) + 매개변수 다형성 + 바인딩(정적/동적)
		//Car[] 1차어ㅜㄴ 배열
		Car[] carList = new Car[3];
...
//		printCar(new Truck());
	...
	public void printCar(Car c) {
		String type = null;
		//매개변수로 전달 받은 c가 참조하는 객체에 따라서 type값을 지정
		...
		// 정적 바인딩 
		// - "프로그램 실행 전" 컴파일 단계에서 메서드 호출부화 수행될 메서드를 묶는 것
		// - 참조변수의 자료형을 기준으로 연결함
		
		// 동적 바인딩
		// - "프로그램 실행 중" 실행할 당시의 객체의 자료형을 기준으로 메서드 호출부와 
		//	수행될 메서드를 묶는 것
