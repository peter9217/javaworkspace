...
//		System.out.println(s2.hashCode());
	...
	private void ex5() {
		...
		//새로운 문자열이 추가되어도 객체의 주소는 변하지않음 == 가변성(mutable)
...
//		Integer(+ 모든 Wrapper)제공 상수 필드
		...
		long l = Long.parseLong("12345678900") + 1000000L;
		System.out.println("l : " + l);
		double d = Double.parseDouble("3.14315926535") + 10.0;
		...
		int iNum2 = iNum1; 
		//  int     integer >> 다르지만 int 대입 됨 [AutoUnboxing]
		System.out.println("iNum2 : " + iNum2);	
		Integer iNum3 = 50;
		//      Integer = int -> Integer [AutoBoxing]
		
		/* Boxing / Unboxing
		* Boxing : 기본 자료형 -> Wrapper Class객체
		* Unboxing : Wrapper Class 객체 -> 기본 자료형
		* 
		* AutoBoxing / AutoUnboxing
		* - 사용자가 신경쓰지 않아도 상황에 따라 기본 자료형 <-> Wrapper Class 객체로 변하는 기술
		* 
		*/
	...
	private void ex8() { 
		...
		Date d2 = new Date(0L); // Date(long date)
								// Date기준 시간으로 부터 ms단위로 지난 시간을 계산해서 저장
		...
		// -> 기준 시간으로 부터 지난 시간 (ms)
		System.out.println("현재 시간 - 기준시간(ms) : " + System.currentTimeMillis());
		//현재 시간 기준으로 1시간 후를 저장
		long temp = 60* 60 * 1000; // 360000ms == 1시간
		//			분	초	 ms
		...
		SimpleDateFormat sdf3 = new SimpleDateFormat(" yyyy년 MM월 dd일 a E요일 hh시 mm분 ss초");
