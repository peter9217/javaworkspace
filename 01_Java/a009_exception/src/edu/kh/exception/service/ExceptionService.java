...
public class ExceptionService {
	...
	public void ex1() throws IOException {
		...
		// readLine 한줄 읽어오기(엔터까지)
		// 빨간줄 이유 readLine() 메서드는 IOException이라고 하는 예외를 발생시킬(던질가능성이 있음
		// 그 상황에 다핸 대비책(예외처리)를 하라고 경고하는 것
		
		System.out.println("입력값 : " + input);
		
		//Checked Exception : 컴파일 단계에서 예외가 발생할 가능성이 있는지 반드시 확인해야 하는 예외
		//	공식 API 문서의 메서드 설명에 throws OOOException으로 작성되어 있는 메서드가 있으면
		//	해당 코드 사용 시 문제가 발생할 것이다 라고 생각하고 그 상황에 대한 예외 처리 코드를 작성
	...
	public void ex3() {
		...
		try {
			...
			//●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●
		}/*catch(Exception e){}*/ //Unreachable catch 여기 밑으로는 의미가 없는 코드
		                          //상위 타입 catch를 뒤쪽에 배치해서 하위 타입 catch에
		                          //대한 검사가 먼저 진행되게 한다.
			...
			//!!!! 주의 사항
			// 	상위 타입을 처리하는 catch문을 하위 타입을 처리하는 catch문 보다 먼저 작성하면
			//	오류 발생
			
			// 스캐너 사용 시 작성법이 잘못되거나 범위를 초과하면 발생
			System.out.println("ㄴㄴ");
		}finally { // try-catch 구문이 끝난 후 마지막으로 수행 [무조건 실행]
	...
	public void ex4() {
		// throw : 예외 강제 발생 ex) throw new IOException();
...
//			e.getMessage();
			e.printStackTrace();
			// Trace : 추적하다
			// -> 예외가 발생한 지점까지의 stack 메모리ㄴ를 추적하여 출력
