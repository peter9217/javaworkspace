...
// 서블릿 매핑에서 유효하지 ㅇ낳은 <url-pattern> [scope]
...
//		RequestDispatcher dispatcher = req.getRequestDispatcher("webapp폴더 기준으로 jsp 경로 작성");
		...
		req.setAttribute("message",  "reqeust scope에 저장된 메시지입니다.");
		// 3. session Scope -> 브라우저당 1개; 브라우저 종료 또는 session 만료까지 유지
		...
		// 4. applicaction -> 서버가 켜져 있는 동안 유지
//		ServletContext application = req.getServletContext();
		...
		req.setAttribute("str", "page 범위에 세팅된 문자열");  
