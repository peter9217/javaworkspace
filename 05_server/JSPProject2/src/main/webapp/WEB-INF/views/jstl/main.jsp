...
<%-- 
    ...
    prefix : 접두사(앞에 붙이는 말, 단어) 태그명 앞에 작성되는 단어 <c:if>
...
<body>
    ...
    <pre>
        JSP에서 자주 사용되거나 공통적으로 사용되는 JAVA코드(if, for, 변수선언, 형변환)
        JAVA 코드(if, for, 변수선언, 형변환)를  스크립틀릿대신 html 태그 형식을 태그화하여 표준으로 제공하는 라이브러리(if, for 간단히 쓰고 싶으면 이 라이브러리를 사용)
    ...
    <h3>JSTL 라이브러리 다운로드 및 등록 방법</h3>
        ...
        <ol>
            <li>
                ...
                <br>
                Standard-1.2.5 -> Jar Files ->  Impl, spen, EL 다운로드
            </li>

            <li>
                /webapp/WEB-INF/lib폴더에 라이브러리 파일(.jar) 추가
            </li>
        ...
        <pre>
            ...
            - 단, escapeXml="true" (기본값)설정 시 html 태그가 해석 X
            - escapeXml="false" : html 태그 해석함
        </pre>
        <c:set var="temp" scope="session" value="<h1>점심시간</h1>" />
        html 태그 해석 안함 : <c:out value="${temp}" escapeXml="false"/>
        <c:out value="${temp}" />
        html 태그 해석 함
