...
//		/* name 속성 값이 같은 파라미터가 여러 개인 경우 String[]로 한 번에 반환받는 getParameterValues("name") 사용*/
		...
		out.println("<html>");
		out.println("<head><title>"+ orderer +" 민의 주문결과</title></head>");
		out.println("<body>");
		...
		out.println("</h3>");
		//옵션이 선택된 경우 출력
		// -> getParameter(), getParameterValues() 수행 시 전달된 값 중 name이 일치하는 값이 없으면 null 반환
		if(optionArr != null) {
			out.println("<ul>");
