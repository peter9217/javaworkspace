...
ALTER TABLE DEPT_COPY ADD CONSTRAINT UK_DEPT_COPY UNIQUE(DEPT_TITLE);

-- DEPT_COPY 테이블의 LOCATION_ID 컬럼에 CHECK 제약조건 추가
-- 컬럼에 작성할 수 있는 값은 L1, L2, L3, L4, L5 
-- 제약조건명 : LOCATION_ID_CHK
ALTER TABLE DEPT_COPY ADD CONSTRAINT CHK_DEPT_COPY CHECK(LOCATION_ID IN ('L1','L2', 'L3', 'L4','L5'));
...
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NOT NULL; NULL 고나련 설정만 MODIFY 사용;
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007571;
--> NOT NULL ADD는 MODIFY 해야하고  삭제는 DROP으로 가능


-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE CONSTRAINT SYS_C007810 /*NOT*/ NULL; --변경 가능
...
SELECT * FROM DEPT_COPY; --> 새로 추가된 컬럼에 기본값 '한국'이 자동 삽입
...
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(10);
...
UPDATE DEPT_COPY SET LNAME = DEFAULT; -- 9행 수정
...
-- TCL : 트랜잭션 제어 언어 (DML 수행 내용을 COMMIT, ROLLBACK, SAVEPOINT)
...
SELECT CASE
	-- 존재하지 않는 사원?
	WHEN (SELECT COUNT(*) FROM EMPLOYEE WHERE EMP_ID = 220) = 0
 	THEN 0
 	--존재하지만 퇴직한 사원?
	WHEN (SELECT COUNT(*) FROM EMPLOYEE WHERE EMP_ID = 220 AND ENT_YN = 'Y') = 1
 	THEN 1
 	-- 둘다 아닌 사원?
 	ELSE 2
END "CHECK" 
...
SELECT COUNT(*) FROM (SELECT * FROM EMPLOYEE WHERE EMP_ID='220' AND ENT_YN='N'
UNION ALL
SELECT * FROM EMPLOYEE WHERE EMP_ID='220');
