...
SELECT * FROM EMPLOYEE2 ORDER BY EMP_ID DESC;
...
INSERT INTO EMP_01 SELECT EMP_ID, EMP_NAME, DEPT_TITLE FROM EMPLOYEE2 LEFT JOIN DEPARTMENT2 ON 
(DEPT_CODE = DEPT_ID);
...
-- DEPARTMENT2 테이블에서 DEPT_TITLE이 '기술지원부'인 부서를 '기술연구팀'으로 변경
UPDATE DEPARTMENT2 SET DEPT_TITLE = '기술연구팀';
SELECT * FROM DEPARTMENT2;
--> 조건을 설정하지 않으면 지정된 테이블의 모든 행이 수정된다.
----> WHERE 작성을 잘하자!!
ROLLBACK;
UPDATE DEPARTMENT2 SET DEPT_TITLE = '기술연구팀' WHERE DEPT_TITLE = '기술지원부';
SELECT * FROM DEPARTMENT2;
COMMIT;
...
UPDATE DEPARTMENT2 SET DEPT_ID = 'D0', DEPT_TITLE = '전략기획2팀' WHERE DEPT_ID = 'D9';
SELECT * FROM DEPARTMENT2;
...
UPDATE EMPLOYEE2 SET BONUS = 0.3 WHERE EMP_ID IN(SELECT EMP_ID
FROM EMPLOYEE2
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
WHERE LOCAL_NAME LIKE 'ASIA%');
