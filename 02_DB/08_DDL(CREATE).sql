...
-- DEFAULT(기본값) : INSERT/UPDATE 시 해당 컬럼의 값을 작성하지 않거나 DEFAULT로 작성하면 DEFAULT 오른쪽에 작성된 값에 컬럼에 반영됨.

-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; --> 딕셔너리 뷰(데이터 딕셔너리에서 일부만 뽑아 보여주는 가상 테이블)
...
INSERT INTO MEMBER VALUES('MEM01', '123ASD', '홍길동', '912345-1234567', SYSDATE);
...
--  데이터  삽입 확인
SELECT * FROM MEMBER;
COMMIT; --> TCL 트랜잭션 저장된 DML 구문을 DB에 반영

ROLLBACK; --> 마지막 COMMIT 상태로 돌아감(트랜잭션에 저장된 DML 삭제)

-------------------------------------------------------
-- 주민등록번호, 전화번호와 같은 숫자로만 이루어진 데이터를 			 --
-- NUMBER 타입으로 지정할 경우의 문제점						 --
-------------------------------------------------------
CREATE TABLE MEMBER2(MEMBER_NAME VARCHAR2(15), MEMBER_SSN NUMBER, MEMBER_PHONE NUMBER);
SELECT * FROM MEMBER2;
...
--    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_UK UNIQUE, -- 컬럼 레벨 제약조건 설정(이름 지정)
    USER_ID VARCHAR2(20),  -- 컬럼 레벨 제약조건 설정(이름 지정)
...
--    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, --컬럼 레벨(제약조건명 O)
    ...
    EMAIL VARCHAR2(50)
    -- 테이블 레발
--    , PRIMARY KEY(USER_NO) -- 제약조건명X
    ,CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO) -- 제약조건명X
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT FRADE_CODE_FK1 REFERENCES USER_GRADE(GRADE_CODE) --<- FOREIGNKEY 라고 안씀
  -- 컬럼 레벨로 FK 제약조건 설정
);
...
--> NULL 사용 가능. (외부 테이블에 참조할 값이 없다 참조X)
...
DELETE FROM USER_GRADE WHERE GRADE_CODE = 20; --> 20번 행은 자식 테이블에서 참조하고 있지 않으므로 삭제가 제한되지 않음
-- GRADE_CODE 중 10은 외래키로 참조되고 있음
DELETE FROM USER_GRADE WHERE GRADE_CODE = 10; --> 10번 행은 자식 테이블에서 참조하고 있음
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, SALARY, PHONE, EMAIL, ENT_DATE, ENT_YN FROM EMPLOYEE LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE) JOIN JOB USING(JOB_CODE) WHERE EMP_ID = '?';
SELECT * FROM EMPLOYEE WHERE EMP_ID = '222' ;
SELECT EMP_NO, EMP_NAME, DEPT_TITLE, JOB_NAME, SALARY, PHONE, EMAIL FROM EMPLOYEE LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE) JOIN JOB USING(JOB_CODE) WHERE ENT_YN='N';
SELECT * FROM EMPLOYEE;
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, HIRE_DATE FROM EMPLOYEE JOIN DEPARTMENT ON(DEPT_ID=DEPT_CODE) ORDER BY HIRE_DATE DESC;
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, HIRE_DATE FROM (SELECT EMP_ID, EMP_NAME, DEPT_TITLE, HIRE_DATE FROM EMPLOYEE JOIN DEPARTMENT ON(DEPT_ID=DEPT_CODE) ORDER BY HIRE_DATE DESC) WHERE ROWNUM<6;
SELECT * FROM DEPARTMENT;
SELECT EMP_NO, EMP_NAME, PHONE, EMAIL, ENT_DATE FROM EMPLOYEE WHERE ENT_YN='Y';
-- ON DELETE SET NUL 삭제 옵션이 적용된 테이블 생성

SELECT DEPT_TITLE, COUNT(DEPT_TITLE), ROUND(AVG(SALARY),0) FROM EMPLOYEE JOIN DEPARTMENT ON(DEPT_ID=DEPT_CODE) GROUP BY DEPT_TITLE, DEPT_ID ORDER BY DEPT_ID;
SELECT DEPT_TITLE, COUNT(DEPT_TITLE), AVG(SALARY) FROM (SELECT DEPT_TITLE, COUNT(DEPT_TITLE), AVG(SALARY), DEPT_ID FROM EMPLOYEE JOIN DEPARTMENT ON(DEPT_ID=DEPT_CODE) GROUP BY DEPT_TITLE, DEPT_ID)
...
--  ,CONSTRAINT (제약조건명) UNIQUE 
  ...
  ,CONSTRAINT GRADE_CODE_FK2 FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE2(GRADE_CODE) ON DELETE SET NULL
);
...
DELETE FROM USER_GRADE2 WHERE GRADE_CODE = 10; -- > 삭제 성공 
...
CREATE TABLE USER_TEST2 (
			...
			USER_ID VARCHAR2(20) CONSTRAINT PK_USER_TEST_ID UNIQUE,
			USER_PWE VARCHAR2(20) CONSTRAINT NN_USER_TEST_PWD NOT NULL,
			PNO VARCHAR2(14) CONSTRAINT UK_PNO NOT NULL,
...
-- 1) 테이블 전체 복사
--CREATE TABLE EMPLOYEE_COPY AS 서브쿼리(RESULT SET)의 형태로 테이블 생성
...
-- 3) 테이블의 형태(컬럼명, 데이터 타입)만 복사(컬럼 값X)
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT FK_DEPT_CODE FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT(DEPT_ID);


-- EMPLOYEE테이블의 DEPT_CODE에 외래키 제약조건 추가
-- 참조 테이블은 DEPARTMENT, 참조 컬럼은 DEPARTMENT의 기본키
ALTER TABLE EMPLOYEE ADD CONSTRAINT FK_JOB_CODE FOREIGN KEY(JOB_CODE) REFERENCES JOB(JOB_CODE);
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT FK_SAL_LEVEL FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE(SAL_LEVEL);


-- DEPARTMENT테이블의 LOCATION_ID에 외래키 제약조건 추가
-- 참조 테이블은 LOCATION, 참조 컬럼은 LOCATION의 기본키
ALTER TABLE DEPARTMENT ADD CONSTRAINT FK_LOCATION_ID FOREIGN KEY(LOCATION_ID) REFERENCES LOCATION;


-- LOCATION테이블의 NATIONAL_CODE에 외래키 제약조건 추가
-- 참조 테이블은 NATIONAL, 참조 컬럼은 NATIONAL의 기본키
ALTER TABLE LOCATION ADD CONSTRAINT FK_NATIONAL_CODE FOREIGN KEY(NATIONAL_CODE) REFERENCES NATIONAL;
