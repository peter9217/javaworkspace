...
-- EMPLOYEE 테이블에서 모든 사원의 이름, 연봉(급여 * 12)
...
-- 별칭 지정 방법
-- 1) 컬럼명 AS 별칭 : 문자만 O, 띄어쓰기 X, 특수문자 X
-- 2) 컬럼명 AS "별칭" : 문자만 O, 띄어쓰기 O, 특수문자 O
-- 3) 컬럼명 별칭 : 문자만 O, 띄어쓰기 X, 특수문자 X
-- 4) 컬럼명 "별칭" : 문자만 O, 띄어쓰기 O, 특수문자 O

-- EMPLOYEE 테이블에서 이름, 연봉 조회
SELECT EMP_NAME AS 사원이름, SALARY * 12 AS 연봉 FROM EMPLOYEE;
SELECT EMP_NAME "사원 이름", SALARY * 12 "연 봉 !" FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서 연봉이 5000만원 이하인 사원의 사번, 이름만 조회
SELECT EMP_ID, EMP_NAME FROM EMPLOYEE WHERE SALARY*12<=50000000;

-- EMPLOYEE 테이블에서 부서코드가 'D9'이 아닌 사원의 사번, 이름, 부서코드 조회
SELECT EMP_ID AS ID, EMP_NAME 이름 , DEPT_CODE 부서 , PHONE 폰 FROM EMPLOYEE WHERE DEPT_CODE <> 'D9';
...
--EMPLOYEE 테이블에서 급여가 300만 이상 500만 이하인 직원의 모든 정보
...
-- EMPLOYEE 테이블에서 급여가 300만 이상 500만 이하가 아닌 직원
--SELECT * FROM EMPLOYEE WHERE SALARY < 3000000 OR SALARY >5000000;
-- NOT BETWEEN A 미만 B초과
SELECT * FROM EMPLOYEE WHERE SALARY NOT BETWEEN 3000000 AND 5000000;


-- EMPLOYEE 테이블에서 부서코드가 'D5' 또는 'D6' 또는 'D9'인 사원의 사번, 이름, 부서코드 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE = 'D5' OR DEPT_CODE = 'D6'OR DEPT_CODE = 'D9';

-- 컬럼명 IN (값1, 값2, 값3, ...) : 컬럼 값에 () 안의 값이 포함된 행은 TRUE
SELECT EMP_ID,EMP_NAME,DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE IN ('D5', 'D6', 'D9');




-- EMPLOYEE 테이블에서 부서코드가 'D5' 또는 'D6' 또는 'D9'이 아닌 사원의 사번, 이름, 부서코드 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE != 'D5' AND DEPT_CODE != 'D6'AND DEPT_CODE != 'D9';
-- 컬럼명 NOT IN (값1, 값2, 값3, ...) : ()내 값을 제외한 행 TRUE 
SELECT EMP_ID,EMP_NAME,DEPT_CODE FROM EMPLOYEE WHERE DEPT_CODE NOT IN ('D5', 'D6', 'D9');
...
-- 연결 연산자( || ) 여러 값을 하나의 컬럼 값으로 연결하는 연산자
-- (== 자바의 문자열 이어쓰기(+))

-- OOO의 급여는 000원 입니다.
SELECT EMP_NAME || '의 급여는 ' || SALARY/10000 || '만원 입니다.' AS 결과 FROM EMPLOYEE;
 ...
 * 
 * '%' (포함)
 * - '%A' : 문자열이 앞은 어떤 문자든 포함되고 마지막은 A로 끝나는 문자열
 * 
 * - 'A%' : A로 시작하는 문자열
 * - '%A%' : A가 포함된 문자열
 * 
...
--EMPLOYEE 테이블에서 전화번호가 '010'으로 시작하는 사원의 사번, 이름, 전화번호 조회
SELECT EMP_ID,EMP_NAME,PHONE FROM EMPLOYEE WHERE PHONE NOT LIKE '010%' OR PHONE IS NULL;

SELECT EMP_ID,EMP_NAME,PHONE FROM EMPLOYEE WHERE PHONE LIKE '010________';


-- EMPLOYEE 테이블에서 이메일 @ 앞글자가 5글자인 사원의 사번, 이름, 이메일 조회
SELECT EMP_ID, EMP_NAME, EMAIL FROM EMPLOYEE WHERE EMAIL LIKE '_____@%';

-- EMPLOYEE 테이블에서 이메일 @ 앞글자가 3글자인 사원의 사번, 이름, 이메일 조회
...
-- <WHERE절 날짜 비교>
-- EMPLOYEE 테이블에서 입사일(고용일)이 '90/01/01' ~ '00/12/31' 사이인 사원의 사번, 이름, 고용일 조회
SELECT EMP_ID, EMP_NAME, HIRE_DATE FROM EMPLOYEE WHERE HIRE_DATE >= '1990-01-01' AND HIRE_DATE <= '2000-12-31';
...
-- 연봉 내림차순 조회
SELECT EMP_NAME, SALARY * 12 AS 연봉 FROM EMPLOYEE ORDER BY /*SALARY * 12*/ /*연봉*/2 DESC;
															-- 컬럼명		/ 별칭/순서
...
-- NULLS FIRST|LAST 확인
-- 전화번호 오름차순 조회
SELECT EMP_NAME, PHONE FROM EMPLOYEE ORDER BY PHONE /*NULLS LAST (오름차순 기본값)*/ NULLS FIRST;

-- 전화번호 내림차순 조회
SELECT EMP_NAME, PHONE FROM EMPLOYEE ORDER BY PHONE DESC; /*NULLS FIRST (내림차순 기본값)*/ 

/* <정렬 중첩>
 * - 큰 분류를 먼저 정렬하고, 내부 분류를 다음에 정렬하는 방식
 */

-- 부서 코드별 급여 내림 차순 (부서코드는 오름차스순)
SELECT EMP_NAME, DEPT_CODE, SALARY FROM EMPLOYEE ORDER BY DEPT_CODE DESC NULLS LAST, SALARY DESC;
