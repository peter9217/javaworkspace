 ...
 *  2) VIEW를 이용한 DML(INSERT, UPDATE, DELETE)이 가능한 경우도 있지만 제약이 많아 조회(SELECT)용도로 대부분 사용.
 * 
 * 
 *  ** VIEW 작성법 **
 * CREATE [OR REPLACE] [FORCE|NOFORCE] VIEW 뷰이름 [컬럼 별칭] AS 서브쿼리(SELECT문)
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME FROM EMPLOYEE LEFT JOIN DEPARTMENT 
ON (DEPT_CODE = DEPT_ID) JOIN JOB USING (JOB_CODE);
--> VIEW 생성
...
SELECT EMP_ID 사번, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명 FROM EMPLOYEE LEFT JOIN DEPARTMENT 
ON (DEPT_CODE = DEPT_ID) JOIN JOB USING (JOB_CODE);

SELECT * FROM V_EMP;

-- V_EMP에서 직급이 대리인 직원의 정보를 조회
...
CREATE VIEW V_DCOPY2 AS SELECT DEPT_ID, LOCATION_ID FROM DEPT_COPY2;
...
-- INSERT 확인
SELECT * FROM V_DCOPY2;
--> VIEW는 값을 저장하지 않는다고 했는데... 저장된 것처럼 보인다. 실제 VIEW에 저장된 것이 아니라 VIEW를 생성할 때 사용한 서브쿼리의 테이블(DEPT_COPY2)
-- 값이 삽입되어있다.

-- 원본 테이블 (DEPT_COPY2) 확인
SELECT * FROM DEPT_COPY2;
--> 원본 테이블에 삽입됨을 확인 INSERT 구문에 포함되지 않은 DEPT_TITLE 컬럼은 NULL이 삽입되어 있다.
...
---> 그런데 DEPT_TITLE에는 NOT NULL 제약조건이 설정되어 NULL값이 삽입 될 수 없음.

-- 데이터 무결성 :중복 X, NULL X -> 신뢰도 높은 데이터
--> 대부분의 컬럼에 NOT NULL 제약조건이 추가되어 있음
---> NOT NULL이 설정된 테이블을 이용해서 VIEW를 만들면 INSERT가 거의 불가능함.
...
-- 읽기전용 X
CREATE OR REPLACE VIEW V_DCOPY2 AS SELECT DEPT_ID, DEPT_TITLE, LOCATION_ID FROM DEPT_COPY2;
-- DEPT_TITLE 포함
...
-- 읽기전용 O
CREATE OR REPLACE VIEW V_DCOPY2 AS SELECT DEPT_ID, DEPT_TITLE, LOCATION_ID FROM DEPT_COPY2 WITH READ ONLY;
-- INSERT 확인
INSERT INTO V_DCOPY2 VALUES('D0', '기획팀', 'L3'); -- > 오류 없이 삽입됨.
 ...
 * 
 * 2) 시퀀스명.CURRVAL : 현재 시퀀스 번호를 얻어옴. 단, 시퀀스가 생성되자마자 호출할 경우 오류 발생
 * 					== 마지막으로 호출한 NEXTVAL 값을 반환
 * 
 * */

-- 테이블 생성 
CREATE TABLE TB_TEST(TEST_NO NUMBER PRIMARY KEY, TEST_NAME VARCHAR2(30) NOT NULL );

-- 시퀀스 생성
CREATE SEQUENCE SEQ_TEST_NO START WITH 100 INCREMENT BY 5 -- 100번부터 5씩 증가
MAXVALUE 150 NOMINVALUE NOCYCLE NOCACHE;--최대값 150 최소값 없음, 반복X, 미리 숫자 안만듬
...
-- UPDATE에 시퀀스 사용
UPDATE TB_TEST SET TEST_NO = SEQ_TEST_NO.NEXTVAL, TEST_NAME = '고길동'||SEQ_TEST_NO.CURRVAL WHERE TEST_NO = (SELECT MAX(TEST_NO) FROM TB_TEST);
--시퀀스 SEQ_TEST_NO.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다 -- 시퀀스 MAX값 초과
SELECT MAX(TEST_NO) FROM TB_TEST; --1
SELECT * FROM (SELECT TEST_NO FROM TB_TEST ORDER BY DESC) ROWNUM ; --2
...
ALTER SEQUENCE SEQ_TEST_NO INCREMENT BY 1 MAXVALUE 155;
...
DROP SEQUENCE SEQ_TEST_NO; -- SEQUENCE 삭제 시퀀스는 START WITH를 변경하고 싶을 때 삭제 후 다시 생성
 ...
 * */
-- INDEX를 사용 X 검색
SELECT * FROM EMPLOYEE WHERE EMP_NAME != '0';
...
-- 인덱스 확인용 테이블 생성
CREATE TABLE TB_INDEX_TEST(TEST_NO NUMBER PRIMARY KEY, TEST_ID VARCHAR2(20) NOT NULL); -- TEST_NO UNIQUE INDEX 자동 생성
...
-- 인덱스 사용 X 조회
SELECT * FROM TB_INDEX_TEST WHERE TEST_ID = 'TEST50000'; -- 80 35 26 26 35 42 


-- 인덱스 사용 O 조회
SELECT * FROM TB_INDEX_TEST WHERE TEST_NO = 500000; -- 3 1 1 0 1 0 

-- TEST_ID 컬럼에 대한 인덱스 생성
CREATE INDEX IDX_TEST_ID ON TB_INDEX_TEST(TEST_ID);

-- 인덱스 생성 후 
SELECT * FROM TB_INDEX_TEST WHERE TEST_ID = 'TEST50000'; -- 53 1 2 1 1 
