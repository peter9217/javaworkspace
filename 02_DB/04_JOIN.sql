...
-- 기존 INNER JOIN 결과에 JOIN 구문 왼쪽에 작성된 EMPLOYEE 테이블의 행 중에서 JOIN결과에 포함되지 않은 행들도 조회하도록 함.
...
-- NULL = D1~D9(+) 왼쪽 테이블 컬럼값과 오른쪽 테이블 컬럼값이 일치하지 않아도 억지로 추가해라
...
SELECT EMP_NAME, DEPT_ID, DEPT_TITLE, DEPT_ID
...
FROM EMPLOYEE /*OUTER*/ FULL JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
...
SELECT EMP_NAME, DEPT_CODE, DEPT_ID, DEPT_TITLE FROM EMPLOYEE CROSS JOIN DEPARTMENT;
...
-- ANSI 표준
SELECT EMP.EMP_ID, EMP.EMP_NAME, NVL(EMP.MANAGER_ID, '없음'), NVL(MGR.EMP_NAME, '없음') FROM EMPLOYEE EMP LEFT JOIN EMPLOYEE MGR ON(EMP.MANAGER_ID = MGR.EMP_ID);


-- 오라클 구문
SELECT EMP.EMP_ID, EMP.EMP_NAME, NVL(EMP.MANAGER_ID, '없음'), NVL(MGR.EMP_NAME, '없음') FROM EMPLOYEE EMP, EMPLOYEE MGR WHERE EMP.MANAGER_ID = MGR.EMP_ID(+);
...
SELECT EMP_NAME, JOB_NAME FROM EMPLOYEE NATURAL JOIN JOB;

-- NATURAL JOIN이 성립되지 않는 경우 0> CROSS JOIN 결과 조회
...
-- JOIN 순서 중요! 1+2+3 앞선 두 테이블(EMPLOYEE, DEPARTMENT)의 조인 후 조인된 결과에 LOCATION 테이블을 조인


-- 오라클 전용
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME FROM EMPLOYEE, DEPARTMENT, LOCATION WHERE DEPT_CODE = DEPT_ID AND LOCATION_ID =LOCAL_CODE;   


-- 조인 순서를 지키지 않은 경우(에러발생)
SELECT * FROM EMPLOYEE, DEPARTMENT, LOCATION WHERE DEPT_CODE = DEPT_ID AND LOCATION_ID =LOCAL_CODE;   
...
-- ANSI
SELECT EMP_NO, EMP_NAME, JOB_NAME DEPT_TITLE, LOCAL_NAME, SALARY FROM EMPLOYEE JOIN JOB USING(JOB_CODE) JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID) JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE) WHERE JOB_NAME = '대리' AND LOCAL_NAME LIKE 'ASIA%';

-- 오라클 전용
SELECT EMP_NO, EMP_NAME, JOB_NAME DEPT_TITLE, LOCAL_NAME, SALARY FROM EMPLOYEE E, JOB J , DEPARTMENT D , LOCATION L WHERE E.JOB_CODE=J.JOB_CODE AND DEPT_CODE=DEPT_ID AND LOCATION_ID = LOCAL_CODE AND JOB_NAME = '대리' AND LOCAL_NAME LIKE 'ASIA%';
...
SELECT EMP_NAME, EMP_NO, DEPT_TITLE, JOB_NAME FROM EMPLOYEE E, DEPARTMENT D, JOB J WHERE E.DEPT_CODE =D.DEPT_ID AND E.JOB_CODE =J.JOB_CODE AND  EMP_NAME LIKE '전%' AND SUBSTR(EMP_NO, 1, 1)='7' AND SUBSTR(EMP_NO, 8, 1)= '2' ; 
      
-- 2. 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 부서명을 조회하시오.
SELECT EMP_NAME, DEPT_TITLE FROM EMPLOYEE E, DEPARTMENT D WHERE E.DEPT_CODE =D.DEPT_ID AND  EMP_NAME LIKE '%형%'; 


-- 3. 해외영업 1부, 2부에 근무하는 사원의 
-- 사원명, 직급명, 부서코드, 부서명을 조회하시오.
SELECT EMP_NAME, JOB_NAME, DEPT_ID, DEPT_TITLE   FROM EMPLOYEE E, DEPARTMENT D, JOB J WHERE E.DEPT_CODE = D.DEPT_ID AND E.JOB_CODE=J.JOB_CODE AND DEPT_TITLE LIKE '%해외영업%'; 

--4. 보너스포인트를 받는 직원들의 사원명, 보너스포인트, 부서명, 근무지역명을 조회하시오.
SELECT EMP_NAME, BONUS, DEPT_TITLE, LOCAL_NAME FROM EMPLOYEE E, DEPARTMENT D, JOB J, LOCATION L WHERE E.DEPT_CODE = D.DEPT_ID AND E.JOB_CODE=J.JOB_CODE AND D.LOCATION_ID =L.LOCAL_CODE AND BONUS IS NOT NULL;

--5. 부서가 있는 사원의 사원명, 직급명, 부서명, 지역명 조회
SELECT EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME FROM EMPLOYEE E, DEPARTMENT D, JOB J, LOCATION L WHERE E.DEPT_CODE = D.DEPT_ID 
AND E.JOB_CODE=J.JOB_CODE AND D.LOCATION_ID =L.LOCAL_CODE AND DEPT_TITLE LIKE '%부%' ;


-- 6. 급여등급별 최소급여(MIN_SAL)를 초과해서 받는 직원들의
--사원명, 직급명, 급여, 연봉(보너스포함)을 조회하시오.
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME,JOB_NAME, SALARY, NVL2(BONUS, (SALARY+SALARY*BONUS)*12, SALARY*12) FROM EMPLOYEE E, JOB J, SAL_GRADE S WHERE  E.JOB_CODE=J.JOB_CODE AND E.SAL_LEVEL=S.SAL_LEVEL AND SALARY > MIN_SAL ORDER BY EMP_ID ;


-- 7.한국(KO)과 일본(JP)에 근무하는 직원들의 
-- 사원명, 부서명, 지역명, 국가명을 조회하시오.
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, LOCAL_NAME 지역명, NATIONAL_NAME 국가명 FROM EMPLOYEE E, DEPARTMENT D,LOCATION L, "NATIONAL" N 
WHERE E.DEPT_CODE = D.DEPT_ID 
AND LOCATION_ID=LOCAL_CODE
AND L.NATIONAL_CODE=N.NATIONAL_CODE
AND NATIONAL_NAME IN ('한국', '일본') ORDER BY EMP_ID;



-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 동료이름을 조회하시오.
-- SELF JOIN 사용
SELECT E1.EMP_NAME, E1.DEPT_CODE, E2.EMP_NAME FROM EMPLOYEE E1, EMPLOYEE E2 WHERE E1.DEPT_CODE = E2.DEPT_CODE AND E1.EMP_ID != E2.EMP_ID ORDER BY 1;
